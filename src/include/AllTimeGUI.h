...
class AllTimeGUI: public sf::Drawable {
  ...
  protected:
    void draw (sf::RenderTarget& target, sf::RenderStates states) const override {
      int maxexp = 5 * pow(main->getLvl(), 2.2) + 25;
      expBlue->setScale({main->getCurrentExp() / float(maxexp) * scale, scale});
      hp->setString(std::to_string(main->getCurrentHP()) + "/" + std::to_string(main->getStats().base_hp));
      mana->setString(std::to_string(main->getCurrentMana()) + "/" + std::to_string(main->getStats().mana));
      gold->setString(std::to_string(main->getCurrentGold()));
      std::string timeString = (game_time->gettime().x < 10? "0" : "") + std::to_string(game_time->gettime().x) + ":" + (game_time->gettime().y < 10? "0" : "") + std::to_string(game_time->gettime().y);
  ...
  public:
    AllTimeGUI (Hero*& hero, GameTime* time): main(hero), game_time(time) {
      ...
      hp = std::make_unique<sf::Text>(font, "", int(scale * 16));
      mana = std::make_unique<sf::Text>(font, "", int(scale * 16));
      gold = std::make_unique<sf::Text>(font, "", int(scale * 16));
      this->time = std::make_unique<sf::Text>(font, "", int(scale * 16));
